...
async function main() {
  ...
  if (!adminExists) {
    const tmpPass = process.env.ADMIN_INIT_PASSWORD;
    if (!tmpPass || !String(tmpPass).trim()) {
      throw new Error(
        'ADMIN_INIT_PASSWORD must be set in server/.env when seeding the first admin user (npm run db:init).'
      );
    }
    const hash = await bcrypt.hash(String(tmpPass).trim(), 12);
    db.prepare('INSERT INTO admin_users (username, password_hash) VALUES (?, ?)').run('admin', hash);
    console.log('Seeded admin user "admin" (password from ADMIN_INIT_PASSWORD).');
  }
